		...
		relativizeData: self.Proxy ? function (data) {
			...
			return new Proxy(data, {
				get: function get(data, property, proxy) {
					...
					if (property in proxy && property in _this3.proxyCache) {
						return _this3.proxyCache[property];
					}
				},

				has: function has(data, property) {
					if (property in data || property in _this3.proxyCache) {
						return true;
					}
					...
					switch (property) {
						case "$index":
							_this3.proxyCache[property] = _this3.index || 0;
							return true; // if index is 0 it's falsy and has would return false!
						case "$next":
						case "$previous":
							if (_this3.closestCollection) {
								_this3.proxyCache[property] = _this3.closestCollection.getData(options)[_this3.index + (property == "$next" ? 1 : -1)];
								return true;
							}

							_this3.proxyCache[property] = null;
							return false;
					}
					...
					if (ret !== undefined) {
						...
						_this3.proxyCache[property] = ret;
					...
					if (property in Mavo.all && isNaN(property) && Mavo.all[property].root) {
						return _this3.proxyCache[property] = Mavo.all[property].root.getData(options);
					}
					...
					var uploadEvents = {
						"paste": function paste(evt) {
							...
							if (item.kind == "file" && item.type.indexOf(type + "/") === 0) {
								...
								var name = prompt(_this.mavo._("filename"), defaultName);
		...
		constructor: function constructor() {
			...
			this.mavo.treeBuilt.then(function () {
				if (!_this.template && !_this.item) {
					...
					_this.item = Mavo.Node.getClosest(_this.element);

					if (_this.item.nodeType === "Primitive" && !_this.item.collection) {
						_this.item = _this.item.parent;
					}
				}
   ...
   */
		...
		getUser: function getUser() {
			...
			return this.request("users/get_current_account", "null", "POST").then(function (info) {
				...
				$.fire(_this2.mavo.element, "mv-login", { backend: _this2 });
			});
		},
